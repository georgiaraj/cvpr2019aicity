...
and smart city applications, the need for accurately analyzing vehicles on urban streets 
by incorporating all the necessary computer vision tasks including detection, classification and
pose estimation as well as re-identification is ever-increasing.
%
Specially, vehicle re-identification attracted increasing attentions in the research community
...
instance across non-overlapping camera views.
%
Due to the unavailability of license plate under uncontrolled view angles,
...
This means that the same identity must be inferred with any given pose/orientation of a vehicle.
(2) Even in the same orientation, vehicles of different identities may look very similar
due to the same vehicle model. This requires vehicle Re-ID models
...
%
However, vehicle ID label {\em alone} can not differentiate the differences between vehicles due to:
(1) vehicle instances of the same ID under different viewpoints,
(2) vehicle instances of different IDs under the same viewpoint.
We suggest that imposing multiple and different supervisory signals
simultaneously (e.g. vehicle ID and vehicle orientation) is more effective
for learning more fine-grained discriminative features for vehicle
Re-ID. 
...
consideration different four tasks including vehicle ID, multi-scale, grayscale, and orientation.
Our model aims to discover and capture concurrently complementary discriminative information.
...
Deep learning techniques have been exploited to vehicle Re-ID. For instance, Liu
...
Each of them is associated with an individual branch of a single model.
...
We employ the ResNet50 \cite{} ({\bf SG: missing reference}) network
