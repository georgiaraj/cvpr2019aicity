...
As we have two training sets, each of which contain different groundtruth labels.

\begin{algorithm}[t]
  \caption{The model training algorithm}
  \label{A:model_training}
  \begin{algorithmic}
    \REQUIRE{Training sets $\mathcal{I}_1$ $\mathcal{I}_2$, labels $\mathcal{Y}_1$\
      $\mathcal{O}$ $\mathcal{L}$ $\mathcal{Y}_2$, model $\mathcal{M}$}
    ...
    \FOR{epoch $e \in (1, E)$}
      ...
      \FOR{each training set $\mathcal{I} \in (\mathcal{I}_1,\mathcal{I}_2)$}
        \STATE{$\bullet$ Feed-forward through model to obtain all branch identity classification \
        predictions on images in $\mathcal{I}$}

        \IF{$\mathcal{I}$ has associated $\mathcal{O}$ and $\mathcal{L}$}
          \STATE{$\bullet$ Feed-forward to obtain orientation classification and landmark regression \
        predictions}
        \ENDIF

        \STATE{$\bullet$ Fuse features and perform consensus identity classification predictions}
        \STATE{$\bullet$ Calculate hard and soft losses for each branch and backpropagate to update weights}
        \STATE{$\bullet$ Calculate hard and soft losses for the consensus branch and backpropagate}
...
\begin{table}
  \begin{tabular}{l || c | c || c | c }
    \hline
    \multirow{2}{*}{Method} & \multicolumn{2}{c ||}{VeRi776} & \multicolumn{2}{c}{VeRi776+VehicleID} \\
    \cline{2-5}
